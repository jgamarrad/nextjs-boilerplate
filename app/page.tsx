...
// Si lo usas más adelante, lo dejo. No rompe nada.
...
> = {
  san_valentin: {
    ...
    ctaText: "Ver catálogo San Valentín",
  ...
  cumpleanos: {
    ...
    ctaText: "Ver catálogo de Cumpleaños",
  ...
  aniversario: {
    ...
    ctaText: "Ver catálogo de Aniversarios",
  ...
  ocasion_especial: {
    ...
    ctaText: "Ver regalos especiales",
...
// Orden estable: primero prioridad desc, y si empatan, por id (para que no “salte” el orden)
...
// Nota: por ahora reutilizas productos, así que los marco como disponibles en varias ocasiones.
// Luego sólo ajustas la propiedad "occasions" por producto.
...
export default function Page() {
  ...
  // 1) Primero los que aplican a la ocasión elegida
  const filtered = productos
    .filter((p) => p.occasions.includes(occasion))
    .slice()
    .sort((a, b) => sortByOccasionPriority(a, b, occasion));

  // 2) Fallback para completar (y que vaya abajo)
  const MIN_ITEMS = 9;
  ...
  const extras = productos
    ...
    .slice()
    // opcional: ordena extras por prioridad para esa ocasión también (si la tienen)
    .sort((a, b) => sortByOccasionPriority(a, b, occasion))
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* Elige la ocasión (sección) */}
      <section className="mx-auto max-w-6xl px-6 pb-10">
        <h2 className="section-title">Elige la ocasión</h2>
      ...
      <section id="catalogo" className="mx-auto max-w-6xl px-6 pb-14">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="section-title gold-text whitespace-nowrap">Catálogo de Regalos</h2>
            <p className="mt-1 text-llama-muted">Te ayudamos a elegir y personalizar tu sorpresa.</p>
          </div>

          <a
            className="btn-secondary gold-border w-full sm:w-auto"
            href={waLink(`Hola, quiero el catálogo de ${current.label} con precios y disponibilidad.`)}
            target="_blank"
            rel="noreferrer"
          >
        ...
        {/* Aviso suave si hubo fallback */}
        <div className="mt-3 text-xs text-white/60">
          Mostrando regalos para <span className="gold-text">{current.label}</span>. Si faltan opciones, completamos con recomendaciones populares.
        </div>
          ...
          {productosMostrar.map((p) => (
            <article key={p.id} className="card p-4">
              <div className="relative overflow-hidden rounded-xl border border-llama-line bg-black/20 aspect-[4/5]">
                <Image src={p.image} alt={p.title} fill className="object-contain" sizes="(max-width: 768px) 100vw, 360px" />
              </div>
